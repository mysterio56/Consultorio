...
<?php if(in_array($permisos,$aPermisos['Agregar']) ): ?>
		<a id="agregar" class="abutton" onclick = "$('.screenLoad').show();" href="<?= base_url('format/agregar') ?>">Agregar</a>
	<?php endif; ?>
		<a id="busavan" class="abutton" onclick = "$('.screenLoad').show();" href="<?= base_url('format/buscar') ?>">Búsqueda Avanzada</a>
		<a id="cancela" display:none onclick = "$('.screenLoad').show();" href="<?= base_url('format') ?>" class="abutton">Cancelar</a>
